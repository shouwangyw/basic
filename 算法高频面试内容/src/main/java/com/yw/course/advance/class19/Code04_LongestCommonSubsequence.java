package com.yw.course.advance.class19;
...
public class Code04_LongestCommonSubsequence {
	...
	private static int process(char[] cs1, char[] cs2, int i, int j) {
		if (i == 0 && j == 0) return cs1[i] == cs2[j] ? 1 : 0;
		else if (i == 0) return cs1[i] == cs2[j] ? 1 : process(cs1, cs2, i, j - 1);
		else if (j == 0) return cs1[i] == cs2[j] ? 1 : process(cs1, cs2, i - 1, j);
		else {
			int p1 = process(cs1, cs2, i - 1, j);
			int p2 = process(cs1, cs2, i, j - 1);
			int p3 = cs1[i] == cs2[j] ? 1 + process(cs1, cs2, i, j - 1) : 0;
