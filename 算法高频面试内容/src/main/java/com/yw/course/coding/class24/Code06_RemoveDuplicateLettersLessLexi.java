...
public class Code06_RemoveDuplicateLettersLessLexi {
    ...
    public String removeDuplicateLetters(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            minAscIdx = s.charAt(minAscIdx) > s.charAt(i) ? i : minAscIdx;
            if (--map[s.charAt(i)] == 0) break;
        }
        return String.valueOf(s.charAt(minAscIdx)) + removeDuplicateLetters(
                s.substring(minAscIdx + 1).replaceAll(String.valueOf(s.charAt(minAscIdx)), "")
        );
    }
