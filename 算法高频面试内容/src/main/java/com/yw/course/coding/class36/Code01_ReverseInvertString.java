...
public class Code01_ReverseInvertString {
	...
	public static char kth(int n, int k) {
		if (lens == null) fillLens();
		if (n == 1) return 'a';
		int half = lens[n - 1];
		if (k <= half) return kth(n - 1, k);
		else if (k == half + 1) return (char) ('a' + n - 1);
		// s(n)找右半区从左往右的第a个 -> 需要找到，s(n-1)从右往左的第a个，当拿到字符之后，invert一下，就可以返回了
		else return invert(kth(n - 1, ((half + 1) << 1) - k));
	}
	// len[s(i)] = len[s(i-1)] + 1 + len(reverse(invert(s(i-1)))) = 2 * len[s(i-1)] + 1
