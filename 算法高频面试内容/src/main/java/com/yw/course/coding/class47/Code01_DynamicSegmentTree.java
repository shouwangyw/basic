...
public class Code01_DynamicSegmentTree {
	...
	public static class DynamicSegmentTree {
		...
		// 当前节点c，表达的范围l~r收到了一个s~e这个任务
		// 返回s~e这个任务影响了多少l~r范围的数
		private int query(Node c, int l, int r, int s, int e) {
			...
			// 1~50  51 ~ 100 
			// 任务s~e  53~76
			if (e <= mid) return query(c.left, l, mid, s, e);
			else if (s > mid) return query(c.right, mid + 1, r, s, e);
			else return query(c.left, l, mid, s, e) + query(c.right, mid + 1, r, s, e);
		}
	}
